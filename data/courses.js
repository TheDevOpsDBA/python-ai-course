      ...
      "sections": [
        ...
        },
        {
          ...
          "examples": [
            {
              "name": "Why venv?",
              "code": "# Why virtual environments matter\n\nprint(\"Without venv (PROBLEM):\")\nprint(\"  Project A needs: requests==2.28\")\nprint(\"  Project B needs: requests==2.31\")\nprint(\"  → Installing one breaks the other!\")\nprint()\nprint(\"With venv (SOLUTION):\")\nprint(\"  Project A/.venv → has requests 2.28\")\nprint(\"  Project B/.venv → has requests 2.31\")\nprint(\"  → Both work independently!\")\nprint()\nprint(\"Commands:\")\nprint(\"  python -m venv .venv        # Create\")\nprint(\"  .venv\\\\Scripts\\\\activate     # Activate (Win)\")\nprint(\"  pip install requests        # Install\")\nprint(\"  pip freeze > requirements.txt  # Save\")\nprint(\"  deactivate                  # Exit\")"
            },
            {
      ...
      "sections": [
          ...
          "examples": [
            {
              ...
              "code": "# Declaring and using variables\nserver_name = \"prod-web-01\"\nip_address  = \"10.0.1.45\"\nport        = 443\nresponse_ms = 142.7\nis_online   = True\n\nprint(\"Server:\", server_name)\nprint(\"IP:    \", ip_address)\nprint(\"Port:  \", port)\nprint(\"Latency:\", response_ms, \"ms\")\nprint(\"Online: \", is_online)"
            },
            {
        ...
        },
        {
          ...
          "examples": [
            {
              "name": "Type Overview",
              "code": "# Exploring Python's core data types\nsamples = {\n    \"hostname\":  \"prod-db-01\",\n    \"port\":      5432,\n    \"cpu_usage\": 67.4,\n    \"is_primary\": True,\n    \"backup_path\": None,\n}\n\nfor key, value in samples.items():\n    print(f\"  {key:<15} = {str(value):<15}  ({type(value).__name__})\")"
            },
            {
          ...
          "examples": [
            {
              ...
              "code": "# Professional formatted output using f-strings\nservers = [\n    (\"web-01\",  88.2, 64.1, \"online\"),\n    (\"web-02\",  42.7, 71.3, \"online\"),\n    (\"db-01\",   31.0, 95.8, \"online\"),\n    (\"cache-01\", 9.4, 22.0, \"offline\"),\n]\n\nprint(f\"{'Host':<12} {'CPU%':>6} {'Mem%':>6} {'Status'}\")\nprint(\"-\" * 38)\nfor name, cpu, mem, status in servers:\n    flag = \" ⚠\" if cpu > 80 or mem > 90 else \"\"\n    print(f\"{name:<12} {cpu:>6.1f} {mem:>6.1f}  {status}{flag}\")"
            },
            {
              "name": "User Input",
      ...
      "sections": [
        ...
        },
        {
          ...
          "examples": [
            {
              "name": "Tuple Basics",
              "code": "# Tuples for fixed configuration data\ndb_config = (\"db-primary.internal\", 5432, \"postgres\")\n\nprint(f\"Host     : {db_config[0]}\")\nprint(f\"Port     : {db_config[1]}\")\nprint(f\"Engine   : {db_config[2]}\")\n\n# Unpacking\nhost, port, engine = db_config\nprint(f\"\\nConnecting to {engine} at {host}:{port}\")\n\n# Tuples in a list (common pattern for tabular data)\nservers = [\n    (\"web-01\", \"10.0.1.10\", 80),\n    (\"web-02\", \"10.0.1.11\", 80),\n    (\"db-01\",  \"10.0.2.10\", 5432),\n]\nfor name, ip, port in servers:\n    print(f\"  {name:<10} {ip:<15} :{port}\")"
            },
            {
              "name": "Immutability",
        ...
        },
        {
          ...
          "examples": [
            {
              "name": "Dict Basics",
              "code": "# Server metadata stored in a dictionary\nserver = {\n    \"name\":        \"prod-web-01\",\n    \"ip\":          \"10.0.1.45\",\n    \"port\":        443,\n    \"region\":      \"eu-west-1\",\n    \"cpu_usage\":   74.2,\n    \"is_healthy\":  True,\n}\n\nprint(\"Server Details:\")\nfor key, value in server.items():\n    print(f\"  {key:<15}: {value}\")\n\n# Safe access with .get()\nbackup = server.get(\"backup_path\", \"Not configured\")\nprint(f\"\\n  {'backup_path':<15}: {backup}\")"
            },
            {
              "name": "Nested Dicts",
          ...
          "examples": [
            {
              ...
              "code": "# Core arithmetic operations\na = 256\nb = 15\n\nprint(f\"a = {a}, b = {b}\")\nprint(f\"a + b  = {a + b}\")\nprint(f\"a - b  = {a - b}\")\nprint(f\"a * b  = {a * b}\")\nprint(f\"a / b  = {a / b:.4f}\")\nprint(f\"a // b = {a // b}  (floor division)\")\nprint(f\"a % b  = {a % b}   (remainder)\")\nprint(f\"2**8   = {2**8}    (2 to the power 8)\")"
            },
            {
          ...
          "examples": [
            {
              ...
              "code": "# Demonstrating all comparison operators\nvalue = 75\n\ncomparisons = [\n    (\"value == 75\",  value == 75),\n    (\"value != 100\", value != 100),\n    (\"value > 80\",   value > 80),\n    (\"value < 80\",   value < 80),\n    (\"value >= 75\",  value >= 75),\n    (\"value <= 74\",  value <= 74),\n    (\"50 <= value <= 100\", 50 <= value <= 100),\n]\n\nfor expr, result in comparisons:\n    print(f\"  {expr:<25} -> {result}\")"
            },
            {
